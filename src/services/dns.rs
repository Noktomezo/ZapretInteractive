        ...
        if let Err(error) = backup_and_apply_dns(&self.runtime_dir) {
            return match restore_dns(&self.runtime_dir) {
                ...
                Err(restore) => Err(anyhow::anyhow!(
                    "{error:#}; откат DNS также не удался: {restore:#}"
                )),
            };
        }
        ...
        match child {
            Ok(child) => {
                    ...
                    fs::write(self.runtime_dir.join("dnscrypt-proxy.pid"), pid.to_string())
                {
                    ...
                    bail!(
                        "не удалось записать dnscrypt-proxy.pid: {error}; остановка: {}; откат DNS: {}",
...
fn write_config(runtime_dir: &Path, endpoint: &str, bootstrap: &[String]) -> Result<PathBuf> {
    ...
    for resolver in bootstrap {
        ...
        if !trimmed.is_empty() {
            bootstrap_array.push(Value::String(trimmed.to_owned()));
        }
    }
    root.insert(
